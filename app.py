...
def result():
    ...
    if request.method == 'POST':
        ...
        if len(trades)>0:
            ...
            detail=f'     {quantity} quantity of {coin} traded at {price} price'
        else:    
        ...
        if len(klines)==0:
            return render_template('home1.html',prediction=result,url_=gauge_json,description=f"{coin} is {safety} to buy.{message}")
        ...
        return render_template('home1.html',prediction=prediction,url=plot_json,url_=gauge_json,description=f"{coin} is {safety} to buy.{message}. Model predicts {prediction} in {str(fg_score)} percent"
